...
async function insertUser() {
  ...
  const { rows } = await query(
    ...
    [uniqueEmail('variantperm-owner'), passwordHash]
  );
  return rows[0].id;
}
...
test('a Manager can set a variant override', async () => {
...
test('a Server cannot set a variant override, but can still read the resolved menu', async () => {
  const { header, shopId } = await setupOwnerWithShop();
  const variant = await createItemWithVariant(header);
