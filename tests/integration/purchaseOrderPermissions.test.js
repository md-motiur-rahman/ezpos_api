...
async function insertUser() {
  ...
  const { rows } = await query(
    ...
    [uniqueEmail('poperm-owner'), passwordHash]
  );
  return rows[0].id;
}
...
test('a Chef (VIEW_INVENTORY only) can read purchase orders but not log them', async () => {
  ...
  const createRes = await request(app)
    ...
    .set('Authorization', chefHeader)
    .send({ supplierId: supplierRes.body.id, items: [{ inventoryItemId: itemRes.body.id, quantity: 10 }] });
  assert.equal(createRes.status, 403);
});

test('a Server (neither permission) cannot read or log purchase orders', async () => {
...
test('the Owner can log purchase orders directly (bypasses the permission system entirely)', async () => {
  const { header, shopId } = await setupOwnerWithShop();
  const supplierRes = await request(app)
    .post(`/api/shops/${shopId}/suppliers`)
    .set('Authorization', header)
